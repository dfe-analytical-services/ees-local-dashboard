...
// this repository, matching where the dashboard lived before it moved to its
// own repository (<checkout>/useful-scripts/dashboard).
export const projectRoot =
  projectRootOverride ??
  path.resolve(__dirname, '../explore-education-statistics');

if (!fs.existsSync(path.join(projectRoot, 'docker-compose.yml'))) {
  throw new Error(
    `'${projectRoot}' doesn't look like an explore-education-statistics ` +
      `checkout (no docker-compose.yml found). Set EES_PROJECT_ROOT to the ` +
      `checkout the dashboard should manage.`,
  );
}
